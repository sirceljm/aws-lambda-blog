...
var get_templates = function(template){
    return {
        ...
        template: require('raw!../../templates/'+template+'/about.html')
    } 
}

exports.handler = (event, context, callback) => {
    const objects_table = event.objects_table || process.env.objects_table;
    const site_base_url = event.site_base_url || process.env.site_base_url;
    const posts_table = event.posts_table || process.env.posts_table;

    const template = event.template || process.env.template;
